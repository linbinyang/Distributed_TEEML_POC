...
void SingleLR(int ndata, int dimension, double* dataset, long *label, double *grad, double* para, double lambda, double learning_rate){
    GradientDescent(grad, ndata, dimension, para, dataset, label, lambda);
    for (int i = 0; i < dimension; i ++){
