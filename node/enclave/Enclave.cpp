...
extern "C" sgx_status_t ecall_calc_buffer_sizes(size_t* epubkey_size, size_t* esealedprivkey_size, size_t* esignature_size, size_t* esealedSK_size, size_t* Outer_size){
  ...
  *esealedSK_size = sgx_calc_sealed_data_size(0U, sizeof(sgx_ec_key_128bit_t));
  *Outer_size = sizeof(sgx_ec256_public_t);
...
extern "C" sgx_status_t new_enclave_init(char *sealed, size_t sealed_size){
	...
	//memset(buf, unsealed_data, unsealed_data_size);
	ret = SGX_SUCCESS;
cleanup:
	if (unsealed_data != NULL){
    	memset_s(unsealed_data, unsealed_data_size, 0, unsealed_data_size);
